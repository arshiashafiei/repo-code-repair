...
def build_vector_store(codebase_root: str="codebase", issues_jsonl_path: str | None="issues/issues.jsonl", batch_size: int = 500, splitter_chunk_size: int = 400) -> VectorStore:
    ...
    embeddings = OllamaEmbeddings(model="granite-embedding:latest", num_thread=3, num_ctx=512, base_url="192.168.1.56:11434")
    
    collection_id = f"{codebase_root}|{issues_jsonl_path}"
    collection_name = f"collection_{make_id(collection_id)[:16]}"
    log.log_and_print(f"Using collection: {collection_name} (for {codebase_root}, {issues_jsonl_path})")
    
    client = chromadb.HttpClient(host="192.168.1.56", port=8088, ssl=False)
    ...
    if existing_count > 0:
        log.log_and_print(f"Collection '{collection_name}' already has {existing_count} documents. Skipping reprocessing.")
        return vector_store_from_client
    
    log.log_and_print(f"Collection '{collection_name}' is empty. Processing documents...")
    ...
    if issues_jsonl_path and Path(issues_jsonl_path).exists():
        with open(issues_jsonl_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                log.log_and_print(f"Processing issue #{issue.get('number')}: {issue.get('title', '')[:50]}...")
                docs.append(
                    Document(
    ...
    for i in range(0, len(chunked_docs), batch_size):
        ...
        log.log_and_print(f"Adding batch {i // batch_size + 1}/{(len(chunked_docs) + batch_size - 1) // batch_size}: documents {i} to {i + len(batch_docs)}")
